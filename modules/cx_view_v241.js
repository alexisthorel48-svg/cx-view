...
function register({ app, q, auth, adminOnly, notifyPlayer }) {
  ...
  app.get('/api/v241/screens/:id/config', auth, async (req, res) => {
    try {
  ...
  app.get('/api/v241/playlists', auth, async (_req, res) => {
    try {
      const r = await q(`SELECT p.id,p.name,p.workspace_id,c.name client_name,
  ...
  app.put('/api/v241/screens/:id/config', adminOnly, async (req, res) => {
    try {
      ...
      const mode = allowed(req.body.player_window_mode, ['WINDOW','FULLSCREEN'], 'WINDOW');
      const layout = allowed(req.body.layout, ['SINGLE','VERTICAL','HORIZONTAL'], 'SINGLE');
      const corner = allowed(req.body.window_corner_mode, ['SQUARE','WINDOWS','ROUNDED'], 'SQUARE');
      ...
      const radius = corner === 'ROUNDED' ? clamp(req.body.window_corner_radius_px, 0, 200, 12) : 0;
      const r = await q(`UPDATE cx_screens SET
        ...
        player_window_mode=$10,display_mode=$11,window_x=$12,window_y=$13,
        window_width_px=$14,window_height_px=$15,window_corner_mode=$16,
        window_corner_radius_px=$17,always_on_top=$18,hide_cursor=$19,
        zone_a_name=$20,zone_b_name=$21,zone_split_percent=$22,
        ...
        WHERE id=$23 RETURNING *`, [
          ...
          mode, mode === 'FULLSCREEN' ? 'KIOSK' : 'WINDOW',
          clamp(req.body.window_x, -32768, 32768, 0), clamp(req.body.window_y, -32768, 32768, 0),
          clamp(req.body.window_width_px, 100, 32768, width), clamp(req.body.window_height_px, 100, 32768, height),
          corner, radius, req.body.always_on_top !== false, req.body.hide_cursor !== false,
